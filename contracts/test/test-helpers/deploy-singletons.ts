...
import { ArtifactRbacModule, ArtifactGnosisSafe, ArtifactGnosisSafeProxyFactory } from "./artifacts";
...
export default async function deploySingletons(deployer: SignerWithAddress) {
  ...
  const rbacModuleAddress = await deploySingleton(factoryAddress, ArtifactRbacModule.bytecode, deployer);
